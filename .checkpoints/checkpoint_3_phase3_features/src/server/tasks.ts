...
export const getTasks = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: projectId }) => {
    const projectTasks = db.tasks.filter((t) => t.projectId === projectId && !t.parentId);
...
export const createTask = createServerFn({ method: 'POST' })
  .validator((data: Omit<Task, 'id' | 'createdAt' | 'updatedAt' | 'reporterId' | 'labels'> & { parentId?: string | null }) => data)
  .handler(async ({ data }) => {
    const newTask: Task = { 
      ...data, 
      id: uuidv4(),
...
export const getSubtasks = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data: parentId }) => {
    const subtasks = db.tasks.filter((t) => t.parentId === parentId);
